...
void DigitsQcTask::initialize(o2::framework::InitContext& /*ctx*/)
{
  ...
  // Super-Module dependent histograms  (as defined in PWGGA/CaloTrackCorrelations/AliAnaCalorimeterQA.cxx
...
void DigitsQcTask::monitorData(o2::framework::ProcessingContext& ctx)
{
  ...
  for (auto digit : digitcontainer) {
    ...
    try {
      ...
      //add a checker fot the invalidCellID and a counter cri
    };
    //fill hfEMod
    //    Int_t    nModule = digit.getModuleNumber(clus);//does it exist?//wich clus?
    //    if(nModule >=0 && nModule < )//how many modules? SM=12 for EMCAL (13, 21 for DCAL)
    //  {
    //    fhEMod->Fill(digit.getEnergy(),nModule);
    // }
  }
}
